...
label RIO_RESTBGM:
    ...
    python hide:
        if 1 <= will_flagbank[willplus.replay_index_ref] <= willplus.replay_index_max:
            if isinstance(willplus.replay_bgm_id_array_base, int):
                tmp_base_addr = willplus.replay_bgm_id_array_base
            else:
                tmp_base_addr = persistent.will_flagbank.name2addr(willplus.replay_bgm_id_array_base)
